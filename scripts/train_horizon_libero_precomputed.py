"""Launch LeRobot training with offline Cosmos latents (no live CosmosAdapter).

Expects the dataset / dataloader to populate ``horizon.precomputed_curr_vis`` and
``horizon.precomputed_future_vis`` (see ``horizon/utils/precomputed_lerobot.py``).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch LeRobot training for HorizonDiT with pre-saved Cosmos latents.",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/train/libero_horizon_precomputed.yaml",
    )
...
def maybe_refresh_default_names(
    ...
    new_suffix = f"{new_suite}_{new_policy_type}_precomputed"
